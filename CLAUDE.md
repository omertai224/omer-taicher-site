...
- **כל דחיפה = מיזוג מיד!** אחרי כל push לבראנצ', ליצור PR ולמזג ל-main בלי לשאול את עומר. עומר רוצה לראות שינויים באתר החי מיד.
...
## Favicon - חובה בכל דף!
**כל דף HTML באתר חייב favicon.**
- שימוש ב-base64 inline SVG (ע כתום על רקע נייבי)
- הקוד: `<link rel="icon" type="image/svg+xml" href="data:image/svg+xml;base64,PHN2ZyB4bWxucz0i...">`
- להוסיף אחרי `<meta charset="UTF-8">`
- כל דף חדש שנוצר חייב לכלול את ה-favicon

## פוטר ונאב אחידים - חובה!
**כל דפי האתר חייבים ניווט ופוטר אחידים.**
- ניווט נטען מ-`assets/nav.html` דרך `nav-loader.js`
- פוטר צריך להכיל: לוגו + לינקים (הרצאות מוקלטות, הדרכות, טיפים, WhatsApp) + זכויות + לינקים משפטיים
- **בעיה ידועה:** הפוטר כתוב inline בכל דף בנפרד ולא אחיד. צריך ליצור footer-loader דומה ל-nav-loader
- **"ההרצאה הבאה" בניווט:** מצביע תמיד להרצאה הבאה (כרגע useful-tools.html). לעדכן כשמשתנה
- **דף הבית יש nav inline משלו** בנוסף ל-nav-loader! צריך לעדכן בשני המקומות

## ניסוח בועות בהדרכות (עדכון אפריל 2026)
**כללים חדשים לניסוח טקסט בבועות:**
- **לא** להשתמש במילה "בהדרכה". להשתמש ב-**"כאן"** במקום
- **לא** לכתוב "במחשב שלכם / כאן" בשתי שורות נפרדות (הוסר!)
- **הניסוח הנכון:** לתאר מה הפעולה עושה, ואז "כאן לחצו על הסימון הכתום" בכתום מודגש
- **דוגמה:** "קליק ימני על הודעה יפתח תפריט מהיר. <span style='color:#f6a67e;font-weight:700;'>כאן לחצו על הסימון הכתום.</span>"
- **לחיצות מרובות (כוכבים):** "ב-Gmail שלכם לוחצים כמה פעמים עד לסימן הנכון. <span style='color:#f6a67e;font-weight:700;'>כאן לחצו פעם אחת על הסימון הכתום.</span>"
- **חסימת קליק ימני + כפולה:** ב-shared/script.js. קליק ימני = "כאן לוחצים רק לחיצה רגילה". לחיצה כפולה = "מספיק לחיצה אחת". הודעה ליד הסמן, נעלמת אחרי 4 שניות

## דפי נחיתה להרצאות (עדכון אפריל 2026)
- **CSS משותף:** `pages/lectures/lecture-landing.css` (717 שורות)
- **כל דף ~95 שורות** תוכן בלבד
- **הרצאות עבר:** badge "ההרצאה התקיימה", בלי כפתור צפייה, בלי תאריכים
- **ההרצאה הבאה (כלים שימושיים):** תאריך + טיימר + שמירת מקום (BizLive 86732)
- **כל דף נחיתה חייב:** hero-bg תמונה, og:image, favicon, tutorials-promo section, about section
- **6 דפים:** main-suspect, useful-tools, surprise-box, things-you-didnt-know, ai-listening, cloud-sync
- **סנכרון לענן:** חסר מ-lectures-data.js (עדיין לא נערך בקמטזיה)

## שקפי howto (איך עובדים עם ההדרכה)
- **2 שקפים:** buildHowToSlide1() + buildHowToSlide2() ב-shared/script.js
- **שקף 1:** 4 כרטיסיות (שקף לחיצה, שקף צפייה, בועת טקסט, רק לחיצה רגילה)
- **שקף 2:** 4 כרטיסיות (זכוכית מגדלת, הקראה בקול, חיצי ניווט, פס ניווט)
- **דף תצוגה מקדימה:** `interactive/tutorials/howto.html`
- **עיצוב:** glass morphism, אנימציות לכל אייקון, רספונסיבי עד 175% zoom
...
## שדרוג הדרכות אינטראקטיביות (עדכון מרץ 2026)
**המטרה:** עומר אומר "תבנה הדרכה על X" והתוצאה מושלמת בלי שאלות.
**קובץ תקלות:** `TROUBLESHOOTING.md` - **לקרוא לפני כל עבודה על הדרכות!**

### עורך הדרכות (omertai.net/editor)
**עורך ויזואלי שרץ על הווב, מוגן ב-GitHub Token.**
- נגיש ב-`omertai.net/editor` או דרך כפתור "עורך הדרכות" בפאנל admin
- משתמש באותו Token של ה-admin (gh_token ב-localStorage)
- שמירה = commit ל-branch העבודה הנוכחי דרך GitHub API
- **כלל ברזל: בתחילת כל סשן, לעדכן את `editor/github.js` שורה 7 (`GH.branch`) לבראנצ' העבודה של הסשן הנוכחי, למזג ל-main מיד, ולסנכרן את הבראנצ'!** ככה עומר שומר מהעורך ישירות לבראנצ' הנכון. Badge בפינה השמאלית תחתונה מראה לאיזה בראנצ' שומרים.
- Claude ממזג ל-main כשעומר מסיים
- זוכר את ההדרכה האחרונה + השקף האחרון בין רענונים
- **רשימת הדרכות:** dropdown עם הדרכות קיימות (Clipboard, Everything, Vibe, Gmail/Schedule, Gmail/Stars)
- **כשמוסיפים הדרכה חדשה:** Claude יוצר את התיקייה ב-GitHub, ההדרכה מופיעה בdropdown אוטומטית
...
### תהליך שדרוג אוטומטי — "תשדרג לי את X" (עדכון מרץ 2026 - בלי FlowShare!)
**FlowShare כבר לא בשימוש!** עומר מצלם מסך ידנית (Print Screen / Snipping Tool) ועובד עם העורך באתר.

**תהליך חדש:**
1. עומר מחליט על הדרכה חדשה ומספר ל-Claude על מה
2. Claude בונה את השלד: תיקייה, index.html, script.js, slides.json עם שקפים מיוחדים
3. עומר מצלם מסך (צילומי מסך של התוכנה/שירות)
4. Claude מקבל את התמונות, מנתח, ובונה את ה-slides.json עם בועות טקסט
5. עומר נכנס לעורך (`omertai.net/editor`), בוחר את ההדרכה, ומדייק: מזיז מסגרות, מתקן טקסטים, מוסיף הערות
6. עומר שומר (כפתור דיסקט) → commit לבראנצ' העבודה הנוכחי (Claude מעדכן את `editor/github.js` בתחילת כל סשן)
7. Claude קורא את השינויים, משפר, ממזג ל-main

**מה כבר לא רלוונטי:**
- ~~שלב 0 ניקוי FlowShare~~ — אין יותר קבצי FlowShare
- ~~חילוץ מ-HTML דחוס~~ — slides.json נבנה מאפס
- ~~שמות FlowShare ארוכות~~ — שמות תמונות [שם]-01.png מההתחלה
- ~~קבצי .flow ו-.fss~~ — לא קיימים יותר

### תהליך צילומי מסך ותמונות (עדכון אפריל 2026)
**עומר מצלם עם PicPick** (צילום מסך מלא + סמן עכבר כתום גדול #ff7c2a).

**שמות קבצים:**
- PicPick שומר עם timestamp: `2026-04-01_01-08-35.png`
- כשעומר מעלה דרך העורך, הקבצים עולים ל-GitHub **עם השמות המקוריים** (בלי שינוי שם!)
- **אחרי שהסדר מאושר**, Claude משנה שמות ל: `[שם]-01.png`, `[שם]-02.png` וכו'
- **הסדר נקבע לפי תאריך יצירת הקובץ** (timestamp), לא לפי שם הקובץ

**תהליך סדר תמונות:**
1. עומר מעלה תמונות דרך העורך (עולות עם שמות מקוריים)
2. עומר שולח צילום מסך של תיקיית הקבצים **ממוין לפי תאריך** כדי שClaude ידע את הסדר
3. Claude מסדר לפי timestamp ומשנה שמות ל-[שם]-01, [שם]-02...
4. Claude בונה slides.json עם בועות טקסט

**סמן העכבר = מיקום הלחיצה:**
- עומר מצלם עם סמן כתום גדול (#ff7c2a) שמצביע על מקום הלחיצה
- Claude משתמש במיקום הסמן כדי למקם את המסגרת הכתומה
- **חשוב:** הסמן לא תמיד מדויק מספיק. עומר מדייק את המסגרות בעורך

**הערות בעורך:**
- עומר מוסיף הערות (notes) בעורך על שקפים שצריכים תיקון
- Claude קורא את ההערות, מבצע, ואז **מוחק את כל ההערות** כדי להתחיל סבב נקי
- הערות הן הוראות חד-פעמיות, לא חלק מההדרכה

**תיבת דואר הדגמות:**
- כל הדרכות Gmail עובדות על תיבת הדגמות: `omertai224demo@gmail.com`
- לקוחות דמה: דני אברהם, שרה כהן, רונית מזרחי (חשבונות walla/outlook)
- התיבה מכילה מיילים מגוונים: תקלות, המלצות, תזכורות, הצעות מחיר
- Claude יש גישה לתיבה דרך Gmail MCP (כשמחובר)

### סוגי הדרכות Gmail
**כל הדרכות Gmail יושבות תחת `interactive/tutorials/Gmail/`:**
- `Gmail/Schedule/` — תזמון שליחה (17 צעדים, מוכן)
- `Gmail/Stars/` — כוכבים וסימון (16 צעדים, בעבודה)
- בעתיד: UndoSend, Filters ועוד

**שתי תבניות:**
- **עם הורדה** (Everything, Vibe) — כולל שקפי הורדה, התקנה, UAC
- **בלי הורדה** (Gmail) — שירות מובנה, בלי התקנה
- ~~שמות FlowShare ארוכות~~ — שמות תמונות [שם]-01.png מההתחלה
- ~~קבצי .flow ו-.fss~~ — לא קיימים יותר
...
- **זיהוי קליק ימני / לחיצה כפולה / לחיצות מרובות:** בניתוח התמונות, אם הפעולה דורשת **קליק ימני**, **לחיצה כפולה (double-click)**, או **לחיצות מרובות** (כמו כוכבים בג'ימייל) — חובה לסמן ב-slide-map. **ניסוח הבועה (עדכון אפריל 2026):**
  - **לא** לכתוב "במחשב שלכם / כאן בהדרכה" בשתי שורות נפרדות (הוסר!)
  - **לא** להשתמש במילה "בהדרכה". להשתמש ב-**"כאן"** במקום
  - **הניסוח הנכון:** לתאר מה הפעולה עושה, ואז "כאן לחצו על הסימון הכתום"
  - **דוגמה לקליק ימני:** "קליק ימני על הודעה יפתח תפריט מהיר. <span style='color:#f6a67e;font-weight:700;'>כאן לחצו על הסימון הכתום.</span>"
  - **דוגמה ללחיצה כפולה:** "לחיצה כפולה על קובץ ההתקנה תתחיל את ההתקנה. <span style='color:#f6a67e;font-weight:700;'>כאן לחצו על הסימון הכתום.</span>"
  - **דוגמה ללחיצות מרובות:** "ב-Gmail שלכם לוחצים על הכוכב כמה פעמים עד שמגיעים ל-V ירוק. <span style='color:#f6a67e;font-weight:700;'>כאן לחצו פעם אחת על הסימון הכתום.</span>"
  - **כל המשפט "כאן לחצו..."** צבוע בכתום (#f6a67e) ומודגש (font-weight:700)
  - **חסימת קליק ימני + כפולה בדפדפן:** ב-shared/script.js יש חסימה אוטומטית. קליק ימני מציג "כאן לוחצים רק לחיצה רגילה". לחיצה כפולה מציגה "מספיק לחיצה אחת". ההודעה קופצת ליד הסמן ונעלמת אחרי 4 שניות
...
- **`right-click`** — קליק ימני עם העכבר. box רגיל + ניסוח: "[פעולה] יפתח תפריט. כאן לחצו על הסימון הכתום."
- **`double-click`** — לחיצה כפולה. box רגיל + ניסוח: "לחיצה כפולה [תעשה X]. כאן לחצו על הסימון הכתום."
...
- **כשמסמנים ושומרים**: CTA מתווסף אוטומטית לסוף הפוסט
  - חינמית: "הדרכה אינטראקטיבית חינמית" + "עברו להדרכה חינמית" → לינק ישיר
  - בתשלום: "הדרכה אינטראקטיבית בתשלום" + "הצטרפו להדרכה" → checkout
- **Badge ברשימת פוסטים**: 🎯 הדרכה אינטראקטיבית חינמית (ירוק) / בתשלום (כתום)
- **WhatsApp**: הודעה מתאימה אוטומטית כשהפוסט כולל הדרכה
- **חץ אחיד**: עיגול עם חץ שמאלה בכל הכפתורים (RTL)
...
#### מוקשים שחוזרים:
- **FlowShare כבר לא בשימוש** — עובדים עם עורך הדרכות (`omertai.net/editor`) + צילומי מסך ידניים
...
**שלב 0: הכנת תשתית (Claude עושה, לא עומר!)**
עומר אומר "תבנה הדרכה על X" ו-Claude מכין:
  - **0א. תיקייה חדשה** — `interactive/tutorials/[שם]/` + תיקיית `images/`
  - **0ב. index.html** — שלד מקושר ל-shared. **נתיב shared לפי עומק!** `../../shared/` (עומק 2) או `../../../shared/` (עומק 3 כמו Gmail/X, Chrome/X)
  - **0ג. script.js** — initApp + buildSlides + buildTextPos. **חובה: `window.bubbleDesignWidth = 853;`**
  - **0ד. slides.json** — **חובה שקף placeholder אחד לפחות!** slides.json ריק = העורך קורס. בלי שקפים מיוחדים (פתיחה/סיום) אלא אם עומר מבקש
  - **0ה. הוספה לעורך** — הוספת ההדרכה ל-`TUTORIAL_CATEGORIES` ב-`editor/loader.js` + הוספת קטגוריה חדשה אם צריך
  - **0ו. דחיפה + מיזוג**
  **תוצאה:** 3 קבצים + תיקיית images/ ריקה. עומר מעלה צילומי מסך.
  **מוקשים (ראו TROUBLESHOOTING.md סעיפים 12-14):**
  - נתיב shared חייב להתאים לעומק (ספור רמות מ-tutorials/)
  - slides.json לא יכול להיות ריק
  - bubbleDesignWidth חייב להיות 853 (כמו editor/render.js)
  - **ניתוח תמונות = כלל ברזל!** כל תמונה מנותחת **אך ורק** לפי מיקום הסמן הכתום + ההקשר של ההדרכה כולה. הסמן קובע מה הפעולה. אם הסמן על Gmail, הטקסט מדבר על Gmail. אם הסמן על שלוש נקודות, הטקסט מדבר על שלוש נקודות. **לא מנחשים, לא מדלגים, לא קופצים.** אם לא רואים את הסמן, לבקש מעומר להגדיל
...
- **שמות תמונות משמעותיים** — [שם]-01.png, ממוספרות לפי סדר
