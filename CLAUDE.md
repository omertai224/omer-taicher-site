...
עומר אומר "תבנה הדרכה על X" ו-Claude מכין:
  - **0א. תיקייה חדשה** — `interactive/tutorials/[שם]/`
  - **0ב. index.html מינימלי** — ~40 שורות שלד, מקושר ל-shared
  - **0ג. script.js מקומי** — רק initApp + buildSlides + buildTextPos (~110 שורות)
  - **0ד. slides.json** — שלד עם שקפים מיוחדים (פתיחה, סרטון, הכירו, [הורדה], סיום)
  - **0ה. soul.md** — נשמת ההדרכה
  - **0ו. דחיפה** — הדרכה מופיעה בעורך
  **תוצאה:** 3 קבצים + תיקיית images/ ריקה. עומר יוסיף צילומי מסך.
