/* Gmail Unsubscribe Tutorial - Dynamic slide builder */
...
function initApp() {
  ...
  if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|mobile|CriOS/i.test(ua)) {
    enhanceMobileBlock('unsubscribe');
    return;
  }
  fetch('slides.json')
    ...
    .catch(function(err) { console.error('Gmail Unsubscribe ERROR:', err); });
}
...
function getPersonalName() {
  ...
  if (name) { localStorage.setItem('unsubscribe_user_display', name); return name; }
  return localStorage.getItem('unsubscribe_user_display') || '';
}
