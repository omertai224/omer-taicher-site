...
var slideMap = {
  ...
  22: { icon: 'finish',   title: 'סיום' }
};

function initApp() {
  var ua = navigator.userAgent || navigator.vendor || window.opera;
  if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|mobile|CriOS/i.test(ua)) {
    enhanceMobileBlock('taskmanager'); return;
  }
  fetch('slides.json')
    ...
    .catch(function(err) { console.error('TaskManager ERROR:', err); });
}
...
function getPersonalName() {
  ...
  if (name) { localStorage.setItem('taskmanager_user_display', name); return name; }
  return localStorage.getItem('taskmanager_user_display') || '';
}
