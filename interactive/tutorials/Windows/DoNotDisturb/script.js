/* Windows DoNotDisturb Tutorial - Dynamic slide builder */
...
function initApp() {
  ...
  if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini|Mobile|mobile|CriOS/i.test(ua)) {
    enhanceMobileBlock('donotdisturb');
    return;
  }
  fetch('slides.json')
    ...
    .catch(function(err) { console.error('DoNotDisturb ERROR:', err); });
}
...
function getPersonalName() {
  ...
  if (name) { localStorage.setItem('dnd_user_display', name); return name; }
  return localStorage.getItem('dnd_user_display') || '';
}
