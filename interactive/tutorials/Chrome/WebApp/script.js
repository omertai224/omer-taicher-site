/* Chrome WebApp Tutorial - Dynamic slide builder */

var slidesData = null;

/* Design width for bubble scale (must match editor/render.js fallback) */
...
function initApp() {
  ...
  fetch('slides.json')
    ...
    .catch(function(err) { console.error('Chrome WebApp ERROR:', err); });
}
...
function getPersonalName() {
  ...
  if (name) { localStorage.setItem('webapp_user_display', name); return name; }
  return localStorage.getItem('webapp_user_display') || '';
}
